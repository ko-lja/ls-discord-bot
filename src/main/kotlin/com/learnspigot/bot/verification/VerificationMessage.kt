...
class VerificationMessage(guild: Guild) {
    init {
        ...
        if (history.isEmpty()) {
            sendVerificationMessage(guild)
        }
    }

    private fun sendVerificationMessage(guild: Guild) {
        Server.verifyChannel.sendMessageEmbeds(
            embed()
                .setTitle("VERIFY YOU OWN THE COURSE")
                .setDescription(createVerificationDescription(guild))
    ...
    private fun createVerificationDescription(guild: Guild): String {
        val getCourseChannel = guild.getTextChannelById(Server.getCourseChannel.id)!!
        return """
            Welcome to the Discord for the LearnSpigot course!
            
            :disappointed: **Don't own the course? See ${getCourseChannel.asMention}**
            ...
            **On Udemy Personal Plan or Udemy For Business?** When verifying, indicate this by typing "Yes" in the provided field. (If you purchased the course directly and don't know what these are, simply answer "No")
        """.trimIndent()
    }
}